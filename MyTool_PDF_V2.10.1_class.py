...
def get_keywords():
    ...
    keywords = [kw.strip() for kw in raw_keywords.splitlines() if
                kw.strip()]  # Mỗi từ khóa là 1 dòng, bỏ trống các dòng không có từ khóa
    return keywords
...
def save_highlighted_pdf():
    ...
    if not keywords_found:
        confirm_save = messagebox.askyesno("Cảnh báo",
                                           "Không có từ khóa nào được tìm thấy và tô màu. Bạn có muốn lưu file không?")
        if not confirm_save:
            return
...
def select_multiple_files():
    ...
    if file_paths:
        ...
        file_listbox.bind("<Double-Button-1>",
                          on_file_click)  # Liên kết sự kiện nhấp chuột với file_listbox để mở file hoặc thư mục
...
under_development_label = tk.Label(under_development_frame, text="Chức năng này đang được phát triển",
                                   font=("Arial", 23))
...
remove_highlight_button = tk.Button(remove_highlight_frame, text="Xóa Highlight các file trên",
                                    command=execute_remove_highlight)
