...
def search_keywords():
    ...
    regex_patterns = build_regex_patterns(keywords)
    found_keywords = {}
    ...
    for keyword, pattern in regex_patterns.items():
        ...
        for page_num in range(len(doc)):
            ...
            for inst in text_instances:
                ...
                highlight.update()
                found_keywords[keyword].append(page_num + 1)

    keywords_found = any(found_keywords.values())
    if keywords_found:
...
def build_regex_patterns(keywords):
    ...
    for keyword in keywords:
        doc = nlp(keyword)
        lemma = " ".join([token.lemma_ for token in doc])  # Sử dụng lemma để nhận diện cơ bản hơn
        pattern = r"\b" + lemma + r"\b"
